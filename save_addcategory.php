...
<?php
    ...
    $octroi=sanitize($con, $_REQUEST["octroi"]);
    $octroi==""?$octroi=0:$octroi=$octroi;

//    echo ("AddEdit:- ".$AddEdit."</br>");
//    echo ("session_userid:- ".$session_userid."</br>");
//    echo ("session_ip:- ".$session_ip."</br>");
//    echo ("categoryname:- ".$categoryname."</br>");
//    echo ("octroi:- ".$octroi."</br>");
//    die();
...
//        mysqli_close($con);
    ...
    if(trim($error_msg)=="") {
        ...
        if ($AddEdit==0) {
            $Procedure = "Call Save_Category('$CurrentDate', $session_userid, '$session_ip', '$categoryname', $octroi);";
        }
        else{
            $IDTableName="category_master";
            $IDColumnName="cmid";
            $IDExist=Check_IDExist($con, $IDTableName, $IDColumnName, $AddEdit);
            if($IDExist>0) {
                $Procedure = "Call Update_Category($IDExist, '$CurrentDate', $session_userid, '$session_ip', '$categoryname', $octroi);";
            }
            else{
                echo("Category ID is not getting. Please contact system administrator....");
            }
        }
...
<script language="javascript">
    ClearAllControls(0);
    show_newlyaddedlist('add_category_2.php', 'div_searchcategory');
</script>
